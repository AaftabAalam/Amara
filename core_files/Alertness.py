...
async def upload_raw_json(request: Request):
    try:
        ...
        included_labels = {"Engagement Words", "Clarifying Words"}
        ...
        pdf_path = "uploads/"+reference_id+"/_Alertness_Report.pdf"
        ...
        with PdfPages(pdf_path) as pdf:
            ...
            if sum(values) > 0:
                ...
                mplt.legend(wedges, legend_labels, title="Categories", loc="best", bbox_to_anchor=(1, 0.4))
                mplt.title('Alertness Report')
                mplt.tight_layout()
                pdf.savefig()
                mplt.close()
            else:
