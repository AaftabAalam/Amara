...
async def upload_raw_json(request: Request):
    try:
        ...
        included_labels = {"Positive Words", "Negative Words"}
        ...
        pdf_path = "uploads/"+reference_id+"/_Emotion_Report.pdf"
        ...
        with PdfPages(pdf_path) as pdf:
            ...
            if sum(values)>0:
                ...
                mplt.legend(wedges, legend_labels, title="Categories", loc="best", bbox_to_anchor=(1, 0.6))
                mplt.title('Emotion Report')
                mplt.tight_layout()
                pdf.savefig()
                mplt.close()
            else:
