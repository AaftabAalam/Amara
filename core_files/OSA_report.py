...
from core_files.metrics import *
...
async def upload_raw_json(request: Request):
    try:
        ...
        result2 = detect_openness(chat_data, openness_keywords)
        result3 = detect_sarcasm(chat_data, sarcasm_indicators, exaggeration_phrases, contradictory_phrases)
        result1 = detect_authenticity(chat_data, authenticity_keywords)
        ...
        included_labels = {"Authenticity Keywords", "Openness Keywords",
                           "Sarcasm Indicators", "Exaggerations", "Contradictions"}
        ...
        pdf_path = "uploads/"+reference_id+"/_OSA_report.pdf"
        ...
        with PdfPages(pdf_path) as pdf:
            ...
            if sum(values) > 0:
                ...
                colours = ["#FF6B6B", "#6BCB77", "#4D96FF", "#FFD93D", "#9B5DE5"]
                ...
                mplt.legend(wedges, legend_labels, title="Categories", loc="best", bbox_to_anchor=(1, 0.7))
                mplt.title('Authenticity, Openness And Sarcasm Report', fontsize=8)
                mplt.tight_layout()
                pdf.savefig()
                mplt.close()
            else:
                ...
                mplt.axis("off")
                mplt.title("Authenticity, Openness And Sarcasm Report")
                pdf.savefig()
                mplt.close()
